...
| 機能 | 内容 |
| --- | --- |
| ランディングページ | クエスト世界観のアプリ紹介。ログイン済み自動リダイレクト（親→/parent、子→/child/[id]）。UDカラー3色（赤:つかう・青:ためる・緑:ふやす）のフィーチャーカード。TOP→ログイン→家族選択の段階的遷移 |
...
| v0.7.4 | 2026-04-10 | 家族削除修正・RLS DELETEポリシー追加。全otetsudaiテーブルにDELETEポリシー追加（anon keyでも削除可能に）、存在しないinvest_portfoliosテーブル参照を削除、React Native移行を見据えたDB側セキュリティ設計 |
