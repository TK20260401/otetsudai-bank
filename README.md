...
| v0.6.1 | 2026-04-09 | じぶんクエストUI改善＆RLS全面修正。プルダウン選択式（プリセット10種＋カスタム）に変更、報酬トップダウン化（親設定基準額）、スタンプ6種をクエスト提案にも追加、メッセージモード簡略化（プレビュー削除・スタンプのみ送信対応）、送信ボタン大型化＋アニメーション。RLS全面修正：全テーブルにanon許可ポリシー追加、親ダッシュボードのクエリにfamilyIdフィルタ追加（他家族データ混入防止）。messageカラムNULLABLE化（スタンプのみ送信対応） |
