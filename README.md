# おこづかいクエスト — お手伝い＝クエスト！マネー冒険アプリ（v0.22.0）
...
| v0.22.0 | 2026-04-20 | **ビジュアライゼーション強化＋Web版SVGコンポーネント拡充**。**Web版新規コンポーネント**: stamp-svg.tsx(17スタンプ×SVGアニメーション)、task-icon-svg.tsx(27キーワード×カテゴリ別アニメーション)、child-character-svg.tsx(性別別ピクセルキャラ+bounce/sway/breatheアニメーション)を新設。**PixelIcons 23種追加**: Dog/Cat/Bath/Plate/Broom/Bed/Car/Shirt/Shoes/Flower/Pot/Backpack/Toothbrush/Trophy/Shop/Paw/Laundry/Sponge/Window/Toilet/Family/Recycle/HeartをWeb版pixel-icons.tsxに追加（モバイル版と同数に統一）。**character-svg armOffset追加**: 全7レベルの腕パーツにarmOffset適用、walk時に足＋腕の連動振りアニメーション実現。**絵文字→SVG置換**: stamp-notifications/approval-dialog/family-stamp-relayの全スタンプ絵文字をStampSvgに置換、child/parent画面のgetTaskIcon絵文字をTaskIconSvgに置換。**ルビ距離調整**: globals.css rtにtranslateY(2px)+ruby-align:center追加 |
| v0.22.1 | 2026-05-14 | **Supabase GRANT モデル対応（2026-05-30 / 10-30 仕様変更への予防的対応）**。**背景**: Supabase は 5/30 以降の新規プロジェクトで public スキーマのテーブルに明示 GRANT を必須化、10/30 に既存プロジェクトへ強制適用。**監査**: 共用プロジェクト `TK20260401's Project` の全 42 テーブルを精査。既存テーブルは全て anon/authenticated/service_role に GRANT 付与済みのため強制適用は無対応で耐える、ただし anon SELECT が全 otetsudai_* テーブルに付いている旧デフォルト挙動を是正。**REVOKE 適用**: 26 個の `otetsudai_*` テーブルから anon ロールの全権限を剥奪（authenticated/service_role は維持）。マイグレーション `20260514_revoke_anon_otetsudai_private.sql`（22 テーブル）と `20260514_revoke_anon_remaining_otetsudai.sql`（残り 4 テーブル）に分割記録。**API 修正**: anon-only クライアントで privateテーブルを叩いていた 2 ルート（`app/api/stock-sync/route.ts` と `app/api/spend-request/route.ts`）を `SUPABASE_SERVICE_ROLE_KEY` 利用に切替、`getSupabaseAdmin()` ファクトリパターンで他の API ルート（family/settings/announcements/admin-setup）と統一。**雛形**: 新規テーブル作成時の標準セット（GRANT パターン A: 認証ユーザーのみ / B: 公開 / C: サーバ専用）を親モノレポ直下 `20260401/SUPABASE-NEW-TABLE-TEMPLATE.sql` に配置 |
