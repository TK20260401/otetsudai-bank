...
export async function checkAndAwardBadges(childId: string): Promise<string[]> {
  ...
  if (!earned.has("earned_1000")) {
    const totalEarned = approvedLogs.reduce((sum: number, l: { task?: { reward_amount: number } }) => sum + (l.task?.reward_amount || 0), 0);
    if (totalEarned >= 1000) newBadges.push("earned_1000");
  }

  // 🔥 3日れんぞく
  if (!earned.has("streak_3") && approvedLogs.length >= 3) {
    const dates = [...new Set(approvedLogs.map((l: { approved_at: string }) =>
